...
let post_handler request =
  ...
  match Yojson.Safe.from_string body |> request_body_of_yojson with
    | Ok data ->
      ...
      (* Validações que determinam confirmar ou cancelar *)
      let amount_float = Float.of_string data.amount in
      let endpoint = 
        if amount_float <= 0.0 then "/cancelar"  (* Amount inválido *)
        else "/confirmar"  (* Amount válido *)
      in
      
      let%lwt (_status, _response) = make_request 
        ("http://127.0.0.1:5001" ^ endpoint)
        body in
